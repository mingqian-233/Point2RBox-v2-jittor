"""mmrotate/models/losses/utils.py 的 Jittor 移植（SAM mask 过滤，纯 numpy/cv2 逻辑）。"""
import cv2
import jittor as jt
import numpy as np
def calculate_shape_metrics(image, mask, required_metrics, original_image=None, aspect_ratio_range=None, prompt_point=None, debug=False):
    ...
    Args:
        ...
        original_image (np.ndarray or jittor.Var, optional): Original image for color consistency calculation
    ...
    if 'color_consistency' in required_metrics and original_image is not None:
        if np.sum(mask) == 0:
            ...
            if isinstance(original_image, jt.Var):
                img_np = original_image.stop_grad().numpy()
